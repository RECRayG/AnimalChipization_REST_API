...
public class VisitedLocationsController {
    ...
    public ResponseEntity<?> searchAnimals(@PathVariable Long animalId, @ModelAttribute VisitedLocationsSearchQuery visitedLocationsSearchQuery) {
        ...
        Pageable pageable = new OffsetBasedPageRequest(
                visitedLocationsSearchQuery.getFrom() != null && visitedLocationsSearchQuery.getFrom() > 0 ? visitedLocationsSearchQuery.getFrom() : 0,
                visitedLocationsSearchQuery.getSize() != null && visitedLocationsSearchQuery.getSize() > 0 ? visitedLocationsSearchQuery.getSize() : 10);
        ...
        if(visitedLocationsSearchQuery.getStartDateTime() != null) {
            startTime = true;
            request.getSearchRequest().add(new SearchRequest("dateTimeOfVisitLocationPoint", visitedLocationsSearchQuery.getStartDateTime().toString()));
        }
        // Обработка критерия поиска "endDateTime", если таковой имеется
        if(visitedLocationsSearchQuery.getEndDateTime() != null) {
            endTime = true;
            request.getSearchRequest().add(new SearchRequest("dateTimeOfVisitLocationPoint", visitedLocationsSearchQuery.getEndDateTime().toString()));
        }
        ...
        List<VisitedLocation> visitedLocations = visitedLocationRepository.findAll(searchSpecification, pageable).getContent();
